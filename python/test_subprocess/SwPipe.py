...
class SwPipe:
    ...
    def __init__(self, cmd, func, exitfunc, readyfunc=None,
        ...
        """
        self._thread = threading.Thread(target=self.__run, args=(cmd, shell, stdin, stdout, stderr, readyfunc))
        self._code = code
        self._func = func
    ...
    def __run(self, cmd, shell, stdin, stdout, stderr, readyfunc):
        """ 私有函数 """
        try:
...
if __name__ == '__main__':  # 那么我们来开始使用它吧
    ...
    def ready():  # 线程就绪反馈函数
        e.write("ls -al")  # 执行
        e.write("ping -c 4 www.baidu.com")
        e.write("echo Hello!World 你好中国！你好世界！")
        e.write("exit")


    e = SwPipe("sh", event, exit, ready)
    e.start()
